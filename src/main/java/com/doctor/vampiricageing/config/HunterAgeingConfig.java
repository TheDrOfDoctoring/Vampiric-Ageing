...
public class HunterAgeingConfig {
    ...
    static {
        ...
        taintedDamageBonuses = COMMON_BUILDER.comment("Damage bonus at each cumulative   age rank").defineList("taintedDamageBonus", Arrays.asList(0f, 0f, 0f, 2f, 2f, 3f, 4f, 4f, 5f, 6f, 7f, 8f), t -> t instanceof Float);
        ...
        taintedBloodMaxHealthIncreases = COMMON_BUILDER.comment("Max health bonus at each age rank").defineList("taintedMaxHealthBonus", Arrays.asList(0f, 0f, 0f, 0f, 0f, 0f, 6f, 6f, 8f, 10f, 12f, 14f), t -> t instanceof Float);
        taintedBloodMovementSpeedIncreases = COMMON_BUILDER.comment("Movement speed bonuses at each age rank").defineList("taintedMovementSpeedIncreases", Arrays.asList(0f, 0f, 0f, 0f, 0f, 0f, 0f, 0f, 0.02f, 0.03f, 0.04f, 0.05f), t -> t instanceof Float);
        taintedBloodTradeDealPricesMultiplier = COMMON_BUILDER.comment("How the price of villager trades are multiplied by baesd on tainted blood age").defineList("taintedBloodtradeDealPricesMultiplier", Arrays.asList(1f, 1f, 1f, 1f, 1f, 1f, 1f, 1.25f, 1.5f, 1.75f, 2f, 2f), t -> t instanceof Float);
